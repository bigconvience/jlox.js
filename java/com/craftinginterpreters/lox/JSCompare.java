    ...
    while (true) {
      ...
      if (tag_is_number(tag1) && tag_is_number(tag2)) {
        if (tag1 == JS_TAG_INT && tag2 == JS_TAG_INT) {
          res = JS_VALUE_GET_INT(op1) == JS_VALUE_GET_INT(op2) ? 1 : 0;
          break;
...
//        }
        ...
        res = js_strict_eq2(ctx, op1, op2, JS_EQ_STRICT) ? 1 : 0;
        break;
...
//    }
  ...
  private static void on_done(JSContext ctx, int res, boolean is_neq, JSValue[] stack_buf, int sp) {
    stack_buf[sp -2] = JS_NewBool(ctx, res ^ (is_neq ? 1 : 0));
  }
  
  private static void on_exception(JSValue[] stack_buf, int sp) {
    stack_buf[sp -2] = JS_UNDEFINED;
    stack_buf[sp -1] = JS_UNDEFINED; 
  }
