    ...
    for (i = 0; i < s.hoisted_def.size(); i++) {
      ...
      if (s.is_global_var && hf.var_name != JS_ATOM_NULL) {
        ...
        for (idx = 0; idx < s.closure_var.size(); idx++) {
          ...
          if (cv.var_name == JSAtomEnum.JS_ATOM__var_.toJSAtom()) {
            dbuf_putc(bc, OPCodeEnum.OP_get_var_ref);
            dbuf_put_u16(bc, idx);
            has_closure = 1;
            force_init = true;
            break;
        ...
        if (has_closure == 0) {
          ...
          if (s.eval_type != LoxJS.JS_EVAL_TYPE_GLOBAL)
            flags |= JS_PROP.JS_PROP_CONFIGURABLE;
          if (hf.cpool_idx >= 0 && !hf.is_lexical) {
            /* global function definitions need a specific handling */
            dbuf_putc(bc, OPCodeEnum.OP_fclosure);
            dbuf_put_u32(bc, hf.cpool_idx);

            dbuf_putc(bc, OPCodeEnum.OP_define_func);
            ...
            if (hf.is_lexical) {
              ...
              if (!hf.is_const)
                flags |= JS_PROP.JS_PROP_WRITABLE;
            }
            dbuf_putc(bc, OPCodeEnum.OP_define_var);
            dbuf_put_u32(bc, hf.var_name);
            dbuf_putc(bc, flags);
          }
        }
      ...
      if (hf.cpool_idx >= 0 || force_init) {
        if (hf.cpool_idx >= 0) {
          dbuf_putc(bc, OPCodeEnum.OP_fclosure);
          dbuf_put_u32(bc, hf.cpool_idx);
          if (hf.var_name == JSAtomEnum.JS_ATOM__default_.toJSAtom()) {
            /* set default export function name */
            dbuf_putc(bc, OPCodeEnum.OP_set_name);
            dbuf_put_u32(bc, JSAtomEnum.JS_ATOM_default);
          }
        } else {
          dbuf_putc(bc, OPCodeEnum.OP_undefined);
        }
        if (s.is_global_var) {
          if (has_closure == 2) {
            dbuf_putc(bc, OPCodeEnum.OP_put_var_ref);
            dbuf_put_u16(bc, idx);
          } else if (has_closure == 1) {
            dbuf_putc(bc, OPCodeEnum.OP_define_field);
            dbuf_put_u32(bc, hf.var_name);
            dbuf_putc(bc, OPCodeEnum.OP_drop);
          } else {
            /* XXX: Check if variable is writable and enumerable */
            dbuf_putc(bc, OPCodeEnum.OP_put_var);
            dbuf_put_u32(bc, hf.var_name);
          }
        } else {
          var_idx = hf.var_idx;
          if ((var_idx & ARGUMENT_VAR_OFFSET) != 0) {
            dbuf_putc(bc, OPCodeEnum.OP_put_arg);
            dbuf_put_u16(bc, var_idx - ARGUMENT_VAR_OFFSET);
          } else {
            dbuf_putc(bc, OPCodeEnum.OP_put_loc);
