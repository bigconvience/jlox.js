...
import static com.lox.javascript.JSThrower.JS_ThrowInternalError;
...
//    print_atom(ctx, name);printf("\n");
    ...
    if (fd.vars.size() >= JSContext.JS_MAX_LOCAL_VARS) {
  ...
  static int add_arg(JSContext ctx, JSFunctionDef fd, JSAtom name)
  {
    ...
    /* the local variable indexes are currently stored on 16 bits */
    if (fd.args.size() >= JSContext.JS_MAX_LOCAL_VARS) {
  ...
  {
    int idx;
    idx = add_var(ctx, fd, JSAtomEnum.JS_ATOM_this.toJSAtom());
