...
import static com.lox.javascript.DynBuf.dbuf_put_u16;
import static com.lox.javascript.DynBuf.dbuf_putc;
import static com.lox.javascript.JSThrower.*;
import static com.lox.javascript.JSVarDef.*;
import static com.lox.javascript.JSVarKindEnum.JS_VAR_NORMAL;
...
public class ScopeVarResolver {
  static int resolve_scope_var(JSContext ctx, JSFunctionDef s,
    ...
    is_pseudo_var = (var_name == JSAtomEnum.JS_ATOM_home_object.toJSAtom() ||
      var_name == JSAtomEnum.JS_ATOM_this_active_func.toJSAtom() ||
      var_name == JSAtomEnum.JS_ATOM_new_target.toJSAtom() ||
      var_name == JSAtomEnum.JS_ATOM_this.toJSAtom());
    ...
    for (idx = s.scopes.get(scope_level).first; idx >= 0; ) {
      ...
      if (vd.var_name == var_name) {
        if (op == OPCodeEnum.OP_scope_put_var.ordinal() || op == OPCodeEnum.OP_scope_make_ref.ordinal()) {
          if (vd.is_const) {
            bc.dbuf_putc(OPCodeEnum.OP_throw_var);
        ...
        var_idx = idx;
        break;
      } else if (vd.var_name == JSAtomEnum.JS_ATOM__with_.toJSAtom() && !is_pseudo_var) {
        bc.dbuf_putc(OPCodeEnum.OP_get_loc);
        dbuf_put_u16(bc, idx);
      ...
      switch (opCode) {
        case OP_scope_make_ref:
          if (is_func_var) {
            ...
            bc.dbuf_putc(OPCodeEnum.OP_object);
            bc.dbuf_putc(OPCodeEnum.OP_get_loc);
            dbuf_put_u16(bc, var_idx);
            bc.dbuf_putc(OPCodeEnum.OP_define_field);
            bc.dbuf_put_u32(var_name);
            bc.dbuf_putc(OPCodeEnum.OP_push_atom_value);
            bc.dbuf_put_u32(var_name);
          } else if (label_done == -1 && can_opt_put_ref_value(bc_buf, ls.pos)) {
            int get_op;
            if ((var_idx & ARGUMENT_VAR_OFFSET) != 0) {
              get_op = OPCodeEnum.OP_get_arg.ordinal();
              var_idx -= ARGUMENT_VAR_OFFSET;
            } else {
              if (s.vars.get(var_idx).is_lexical)
                get_op = OPCodeEnum.OP_get_loc_check.ordinal();
              else
                get_op = OPCodeEnum.OP_get_loc.ordinal();
            }
            ...
            if ((var_idx & ARGUMENT_VAR_OFFSET) != 0) {
              bc.dbuf_putc(OPCodeEnum.OP_make_arg_ref);
              bc.dbuf_put_u32(var_name);
              dbuf_put_u16(bc, var_idx - ARGUMENT_VAR_OFFSET);
            } else {
              bc.dbuf_putc(OPCodeEnum.OP_make_loc_ref);
              bc.dbuf_put_u32(var_name);
              dbuf_put_u16(bc, var_idx);
            }
          }
        ...
        case OP_scope_put_var_init:
          is_put = (op == OPCodeEnum.OP_scope_put_var.ordinal() || op == OPCodeEnum.OP_scope_put_var_init.ordinal());
          if ((var_idx & ARGUMENT_VAR_OFFSET) != 0) {
            bc.dbuf_putc(OPCodeEnum.OP_get_arg.ordinal() + (is_put ? 1 : 0));
            dbuf_put_u16(bc, var_idx - ARGUMENT_VAR_OFFSET);
            /* XXX: should test if argument reference needs TDZ check */
          } else {
            if (is_put) {
              if (s.vars.get(var_idx).is_lexical) {
                if (op == OPCodeEnum.OP_scope_put_var_init.ordinal()) {
                  /* 'this' can only be initialized once */
                  if (var_name == JSAtomEnum.JS_ATOM_this.toJSAtom())
                    bc.dbuf_putc(OPCodeEnum.OP_put_loc_check_init);
                  else
                    bc.dbuf_putc(OPCodeEnum.OP_put_loc);
                } else {
                  bc.dbuf_putc(OPCodeEnum.OP_put_loc_check);
                }
              } else {
                bc.dbuf_putc(OPCodeEnum.OP_put_loc);
              }
              ...
              if (s.vars.get(var_idx).is_lexical) {
                bc.dbuf_putc(OPCodeEnum.OP_get_loc_check);
              } else {
                bc.dbuf_putc(OPCodeEnum.OP_get_loc);
              }
            }
        ...
        case OP_scope_delete_var:
          bc.dbuf_putc(OPCodeEnum.OP_push_false);
          break;
      }
      on_resolve_done(label_done, bc, s);
      return pos_next;
    ...
    for (fd = s; fd.parent != null; ) {
      ...
      if (scope_level == 0) {
        ...
        for (idx = fd.scopes.get(scope_level).first; idx >= 0; ) {
          ...
          if (vd.var_name == var_name) {
            ...
            if (idx >= 0) {
              bc.dbuf_putc(OPCodeEnum.OP_get_var_ref);
              dbuf_put_u16(bc, idx);
              bc.dbuf_putc(get_with_scope_opcode(op));
              bc.dbuf_put_u32(var_name);
              label_done = s.new_label_fd(label_done);
      ...
      if (var_name == JSAtomEnum.JS_ATOM_arguments.toJSAtom() && fd.has_arguments_binding) {
      ...
      if (fd.var_object_idx >= 0 && !is_pseudo_var) {
        ...
        idx = get_closure_var(ctx, s, fd, false,
          fd.var_object_idx, JSAtomEnum.JS_ATOM__var_.toJSAtom(),
    ...
    if (var_idx < 0 && fd.is_eval) {
      ...
      for (idx1 = 0; idx1 < fd.closure_var.size(); idx1++) {
        ...
        } else if ((cv.var_name == JSAtomEnum.JS_ATOM__var_.toJSAtom() ||
          cv.var_name == JSAtomEnum.JS_ATOM__with_.toJSAtom()) && !is_pseudo_var) {
          int is_with = (cv.var_name == JSAtomEnum.JS_ATOM__with_.toJSAtom()) ? 1 : 0;
          if (fd != s) {
            idx = get_closure_var2(ctx, s, fd,
              false,
              cv.is_arg, idx1,
    ...
    switch (opCode) {
      case OP_scope_make_ref:
        if (label_done == -1 && can_opt_put_global_ref_value(bc_buf, ls.pos)) {
          ...
          bc.dbuf_putc(OPCodeEnum.OP_make_var_ref);
          bc.dbuf_put_u32(var_name);
        }
        break;
      case OP_scope_get_ref:
        ...
        bc.dbuf_putc(OPCodeEnum.OP_undefined);
        bc.dbuf_putc(OPCodeEnum.OP_get_var);
        bc.dbuf_put_u32(var_name);
        break;
      case OP_scope_get_var_undef:
      case OP_scope_get_var:
      case OP_scope_put_var:
        bc.dbuf_putc(OPCodeEnum.OP_get_var_undef.ordinal() + (op - OPCodeEnum.OP_scope_get_var_undef.ordinal()));
        bc.dbuf_put_u32(var_name);
        break;
      case OP_scope_put_var_init:
        bc.dbuf_putc(OPCodeEnum.OP_put_var_init);
        bc.dbuf_put_u32(var_name);
        break;
      case OP_scope_delete_var:
        bc.dbuf_putc(OPCodeEnum.OP_delete_var);
        bc.dbuf_put_u32(var_name);
        break;
    }
    on_resolve_done(label_done, bc, s);
  ...
  private static int on_has_idx(JSContext ctx, OPCodeEnum opCode, JSFunctionDef s, DynBuf bc, JSAtom var_name,
                                ...
                                LabelSlot ls) {
    if ((opCode == OPCodeEnum.OP_scope_put_var || opCode == OPCodeEnum.OP_scope_make_ref) &&
    ...
    switch (opCode) {
      case OP_scope_make_ref:
        if (is_func_var) {
          ...
          if (s.closure_var.get(idx).is_lexical)
            get_op = OPCodeEnum.OP_get_var_ref_check.ordinal();
          else
            get_op = OPCodeEnum.OP_get_var_ref.ordinal();
          pos_next = optimize_scope_make_ref(ctx, s, bc, bc_buf, ls,
            pos_next,
            get_op, idx);
        } else {
          ...
          bc.dbuf_putc(OPCodeEnum.OP_make_var_ref_ref);
          bc.dbuf_put_u32(var_name);
          dbuf_put_u16(bc, idx);
        }
        break;
      ...
      case OP_scope_put_var:
      case OP_scope_put_var_init:
        boolean is_put = (opCode == OPCodeEnum.OP_scope_put_var ||
          opCode == OPCodeEnum.OP_scope_put_var_init);
        if (is_put) {
          if (s.closure_var.get(idx).is_lexical) {
            if (opCode == OPCodeEnum.OP_scope_put_var_init) {
              /* 'this' can only be initialized once */
              if (var_name == JSAtomEnum.JS_ATOM_this.toJSAtom())
                bc.dbuf_putc(OPCodeEnum.OP_put_var_ref_check_init);
              else
                bc.dbuf_putc(OPCodeEnum.OP_put_var_ref);
            } else {
              bc.dbuf_putc(OPCodeEnum.OP_put_var_ref_check);
            }
          } else {
            bc.dbuf_putc(OPCodeEnum.OP_put_var_ref);
          }
          ...
          if (s.closure_var.get(idx).is_lexical) {
            bc.dbuf_putc(OPCodeEnum.OP_get_var_ref_check);
          } else {
            bc.dbuf_putc(OPCodeEnum.OP_get_var_ref);
          }
        }
  ...
  private static void on_resolve_done(int label_done, DynBuf bc, JSFunctionDef s) {
    if (label_done >= 0) {
      bc.dbuf_putc(OPCodeEnum.OP_label);
      bc.dbuf_put_u32(label_done);
  ...
  static int add_closure_var(JSContext ctx, JSFunctionDef s,
    ...
    if (s.closure_var.size() >= JSContext.JS_MAX_LOCAL_VARS) {
  ...
  static int get_with_scope_opcode(int op) {
    if (op == OPCodeEnum.OP_scope_get_var_undef.ordinal())
      return OPCodeEnum.OP_with_get_var.ordinal();
    else
      return OPCodeEnum.OP_with_get_var.ordinal() + (op - OPCodeEnum.OP_scope_get_var.ordinal());
  }

  static boolean can_opt_put_ref_value(final byte[] bc_buf, int pos) {
    OPCodeEnum opcode = JUtils.get_opcode(bc_buf, pos);
    return (JUtils.get_opcode(bc_buf, pos + 1) == OPCodeEnum.OP_put_ref_value &&
      (opcode == OPCodeEnum.OP_insert3 ||
        opcode == OPCodeEnum.OP_perm4 ||
        opcode == OPCodeEnum.OP_nop ||
        opcode == OPCodeEnum.OP_rot3l));
  }
  ...
  static int optimize_scope_make_ref(JSContext ctx, JSFunctionDef s,
    ...
    /* replace the reference get/put with normal variable
       accesses */
    if (get_opcode(bc_buf, pos_next) == OPCodeEnum.OP_get_ref_value) {
      dbuf_putc(bc, get_op);
      dbuf_put_u16(bc, var_idx);
      pos_next++;
    }
     ...
     */
    end_pos = label_pos + 2;
    if (get_opcode(bc_buf, label_pos) == OPCodeEnum.OP_insert3)
      put_u8(bc_buf, pos++, OPCodeEnum.OP_dup);
    put_u8(bc_buf, pos, get_op + 1);
    put_u16(bc_buf, pos + 1, var_idx);
    pos += 3;
    /* pad with OP_nop */
    while (pos < end_pos)
      put_u8(bc_buf, pos++, OPCodeEnum.OP_nop);
    return pos_next;
  }
  ...
  static int optimize_scope_make_global_ref(JSContext ctx, JSFunctionDef s,
    ...
    if (is_strict) {
      ...
      bc.dbuf_putc(OPCodeEnum.OP_check_var);
      bc.dbuf_put_u32(var_name);
    } else {
    ...
    label_pos = ls.pos;
    pos = label_pos - 5;
    assert (get_opcode(bc_buf, pos) == OPCodeEnum.OP_label);
    end_pos = label_pos + 2;
    op = get_opcode(bc_buf, label_pos);
    if (is_strict) {
      if (op != OPCodeEnum.OP_nop) {
        switch (op) {
          case OP_insert3:
            op = OPCodeEnum.OP_insert2;
            break;
          case OP_perm4:
            op = OPCodeEnum.OP_perm3;
            break;
          case OP_rot3l:
            op = OPCodeEnum.OP_swap;
            break;
          default:
            break;
        }
      ...
      if (op == OPCodeEnum.OP_insert3)
        put_u8(bc_buf, pos++, OPCodeEnum.OP_dup);
    }
    if (is_strict) {
      put_u8(bc_buf, pos, OPCodeEnum.OP_put_var_strict);
      /* XXX: need 1 extra OP_drop if destructuring an array */
    } else {
      put_u8(bc_buf, pos, OPCodeEnum.OP_put_var);
